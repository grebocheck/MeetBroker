...
24 години і ставлять системний лист у transactional outbox. У цьому режимі
потрібно налаштувати `SMTP_HOST` та інші `SMTP_*`; без SMTP API поверне явну
помилку й не створюватиме запит, який неможливо підтвердити. Зміна змінної
вимагає перезапуску `api` та `worker`.

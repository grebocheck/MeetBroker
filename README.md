...
`.env.example` запускає застосунок у конкурсному режимі `APP_MODE=DEMO`.
Новий користувач після реєстрації не очікує корпоративного схвалення й може
одразу відтворити основний сценарій ТЗ. У бойовому середовищі встановіть
`APP_MODE=PRODUCTION`: тоді новий профіль повинен окремо схвалити
...
не ввімкнув email-розсилку. Для нових профілів автоматично активний лише
внутрішній канал застосунку; email і Telegram користувач вмикає явно у
власних налаштуваннях. Системні листи підтвердження адреси не залежать від
цієї матриці. Для реальної інсталяції також потрібно змінити паролі БД,
webhook secret і не використовувати демонстраційні облікові дані.

Щоб повторно застосувати seed до вже запущеного Compose-стека:

```bash
docker compose exec api npm run db:seed:prod --workspace @meetbroker/api
```

Для локально запущеного API з доступним `DATABASE_URL` використовуйте
`npm run db:seed`. Команда є повторюваною для demo-профілів і показових
бронювань.
...
Операторський backup охоплює PostgreSQL і persistent uploads, створює
контрольні SHA-256 суми та фіксує поточний Git commit. Каталог призначення
задається явно:

```bash
scripts/backup.sh /srv/backups/meetbroker
```

Для перевірки відновлення використовуйте окремий тестовий Compose project
або окрему інсталяцію. Відновлення бойового стека є навмисно захищеною
руйнівною операцією:

```bash
MEETBROKER_RESTORE_CONFIRM=restore \
  scripts/restore.sh /srv/backups/meetbroker/meetbroker-backup-YYYYMMDDTHHMMSSZ
```

Перед заміною даних команда перевіряє checksum та обидва формати архівів,
зупиняє API й зовнішню доставку, а після відновлення запускає застосунок із
`SEED_DEMO_DATA=false`. Worker залишається зупиненим, щоб оператор спочатку
переглянув чергу невдалих/очікуваних доставок в адмініструванні; після
перевірки його потрібно ввімкнути окремо через
`docker compose up -d worker`. Backup слід зберігати зашифрованим, поза
сервером застосунку, і регулярно перевіряти повним тестовим restore.
...
- запрошення, відповіді учасників і відкриті заходи із серверним пошуком та
  пагінацією;
- профілі, власні фото та готові аватари;
- перевірка та автоматична WebP-оптимізація аватарів, зображень кімнат і
  подій із єдиним вхідним лімітом 12 МБ на API та reverse proxy;
...
- центр сповіщень, брендовані email і багатомовні Telegram-повідомлення з
  категоріями та емодзі через transactional outbox;

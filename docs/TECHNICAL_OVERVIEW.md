...
**Стан продукту:** MeetBroker v0.2.0, опублікований реліз
...
Пошукові запити використовують параметризований `ILIKE` із буквальним
екрануванням `%`, `_` і `\`. Міграція `015` додає `pg_trgm` GIN-індекси
для назв, email і кімнат, а також частковий індекс майбутніх відкритих
подій та складені індекси attendee/outbox-проєкцій. План відкритих подій
підтверджено `Index Only Scan`, а title search — `Bitmap Index Scan`.
